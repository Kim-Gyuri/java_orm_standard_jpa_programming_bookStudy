...
+ memberA를 em.persist()를 해서 넣으면, 먼저 1차캐시에 들어간다.
+ (들어가는 동시에) JPA가 memberA 엔티티를 분석해서 INSERT SQL한다.
+ 그래서 쓰기지연 sQL 저장소에 쌓아둔다.
+ 그 다음 memberB를 persist한다.
+ (이때도 1차 캐시에 memberB를 넣고, <br> 동시에 INSERT SQL을 생성해서 쓰기지연SQL저장소에 쌓아둔다.)
...
+ "----" 선 이후에, 쿼리가 DB에 간다. insert쿼리는 2개가 나간다
