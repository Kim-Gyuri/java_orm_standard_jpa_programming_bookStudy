...
> `참고1` <br> 자바의 기본 타입은 절대 공유되지 않는다.  <br> 예: a=b (b의 값을 복사해서 a에 입력한다.) <br> 
...
> `참고2` <br> Integer처럼 래퍼 클래스나 String 같은 특수한 클래스도 있다. <br> 이것들은 객체지만 자바언어에서 기본 타입처럼 사용할 수 있게 지원하므로 기본값 타입으로 정의했다. <br>
...
System.out.println(b); //20 래퍼런스가 넘어가서 같은 인스턴스를 공유한다.
```
